...
contains
  ...
  subroutine init_basis_set(V,nvec,m,molecule,tag)
    ...
    if(tag.eq.'rand') then
       ...
       do i=1,nvec
          call dcopy(m%N,molecule%wf%wfc(:,i),1,Vdump(:,i),1) ! g->d
       end do
    end if
 ...
 !   double precision, external :: ddot
    ...
    if (param%init_wf)   then
       ...
       call init_basis_set(V,nvec,mesh,molecule,'wfc')
          !      call check_ortho(V,nvec,mesh%N)
          !       stop
    end if
...
!       cvg%ETA=param%ETA
  ...
  subroutine davidson_step(nvec,V,m,nvecmin,iloop,cvg,pot,time_spent,molecule,param)
    ...
    call cpu_time(time_spent%start_loc)
    call compute_residual(residual,VRitz,molecule%wf%eps,nvec,cvg,m,pot%tot)
  ...
  subroutine compute_residual(r,VRitz,S,nvec,cvg,m,pot_tot)
    ...
    do j=1,nvec
       ...
       if(j.le.cvg%nwfc) then
          cvg%wfc(j)%resi=m%dv*ddot(m%N,r(:,j),1,r(:,j),1)
