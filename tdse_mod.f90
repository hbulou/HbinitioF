...
contains
    subroutine tdse(molecule,cvg,param)
      ...
      do i=1,molecule%mesh%nactive
         write(1,*) i*molecule%mesh%dx*a0,dreal(wfc(i,oldt)),dimag(wfc(i,oldt)),abs(wfc(i,oldt))
      end do
      close(1)
      ...
      do i=1,molecule%mesh%nactive
         write(1,*) a0*i*molecule%mesh%dx,molecule%pot%tot(i)
      end do
      close(1)
      ...
      do time_loop=1,1000000
         ...
         do i=1,molecule%mesh%nactive-1
          write(1,*) a0*i*molecule%mesh%dx,dreal(wfc(i,newt)),dimag(wfc(i,newt)),abs(wfc(i,newt))
       end do
       close(1)
